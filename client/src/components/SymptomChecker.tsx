  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-6">
        <div className="p-3 rounded-2xl bg-primary/20 text-primary border border-primary/30 shadow-lg">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full h-14 pl-12 pr-4 rounded-xl glass border-primary/20 focus:ring-primary/40 focus:border-primary bg-slate-900/30 text-lg shadow-inner"
        />
      </div>
      ...
      <select
        ...
        className="w-full rounded-2xl glass border-primary/20 focus:ring-primary/40 focus:border-primary bg-slate-900/50 text-foreground cursor-pointer overflow-hidden p-2"
      >
        <option value="" className="p-4 bg-slate-900 text-muted-foreground">
        ...
        {filteredSymptoms.map((sym) => (
          <option
            key={sym}
            value={sym}
            className="p-4 bg-slate-900 hover:bg-primary/20 transition-colors"
          >
            {sym}
          </option>
        ))}
      ...
      {result && (
        ...
        >
          <div className="text-left border-b border-white/10 pb-4 flex justify-between items-center">
            <div>
              <p className="text-xs text-primary font-bold uppercase tracking-widest mb-1">
                Recommended Dept
              </p>
              <p className="text-2xl font-black">{result.dept}</p>
            </div>
            <button
              onClick={() =>
                handleSpeak(
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Govt Option */}
            <div className="p-5 bg-green-500/10 border border-green-500/20 rounded-2xl shadow-sm">
              <p className="text-xs text-green-400 font-bold uppercase mb-3 tracking-wider">
                Government (Free/Low Cost)
              </p>
              <p className="text-xl font-bold mb-4">{result.govt}</p>
              <div className="flex gap-3">
                <a
                  href={`tel:${result.govtPhone}`}
                  className="flex-1 flex items-center justify-center gap-2 p-3 bg-green-500/20 hover:bg-green-500/30 rounded-xl text-green-400 transition-all font-bold"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 p-3 bg-white/5 hover:bg-white/10 rounded-xl text-white transition-all font-bold"
                >
            ...
            {/* Private Option */}
            <div className="p-5 bg-blue-500/10 border border-blue-500/20 rounded-2xl shadow-sm">
              <p className="text-xs text-blue-400 font-bold uppercase mb-3 tracking-wider">
                Private (Best Quality)
              </p>
              <p className="text-xl font-bold mb-4">{result.private}</p>
              <div className="flex gap-3">
                <a
                  href={`tel:${result.privatePhone}`}
                  className="flex-1 flex items-center justify-center gap-2 p-3 bg-blue-500/20 hover:bg-blue-500/30 rounded-xl text-blue-400 transition-all font-bold"
                >
                  <Phone className="w-4 h-4" /> Call
                </a>
                <a
